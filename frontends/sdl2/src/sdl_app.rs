...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let mut audio_queue_state =
        SdlAudioQueueState::new(session.audio_sample_rate_hz(), last_host_tick);
    let mut save_autosave = SaveAutosaveDebouncer::new(SAVE_AUTOSAVE_DEBOUNCE);

    'main_loop: loop {
        let now = Instant::now();
        session.push_host_time(now.saturating_duration_since(last_host_tick));
        last_host_tick = now;

        for event in event_pump.poll_iter() {
            match process_event(session.gameboy_mut(), event) {
        ...
        refill_audio_queue(&audio_queue, &mut session, &mut audio_queue_state, now);

        if save_autosave
            .update_and_should_flush(session.gameboy().cartridge_battery_save_dirty(), now)
        {
            flush_persistence(&persistence, session.gameboy_mut())?;
            save_autosave.mark_flushed();
        }
        ...
        if !produced_frame {
            let sleep_for = session.duration_until_next_frame();
