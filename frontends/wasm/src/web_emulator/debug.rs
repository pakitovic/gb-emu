...
impl WebEmulator {
    pub fn serial_output(&self) -> String {
        self.session.gameboy().serial_output().to_string()
    }

    pub fn rom_title(&self) -> String {
        self.session.gameboy().rom_title().to_string()
    }

    pub fn cartridge_debug_report(&self) -> String {
        format_cartridge_debug_report(&self.session.gameboy().cartridge_metadata())
    }
    ...
    pub fn cartridge_has_battery_save(&self) -> bool {
        let metadata = self.session.gameboy().cartridge_metadata();
