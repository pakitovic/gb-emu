...
- DMA scheduling scaffolding is currently DMG-only and implements only OAM DMA transfer rules; `HDMA1..HDMA5` writes are decode-only scaffolds (DMG-visible no-ops with shadowed register/request-mode state), and future CGB DMA modes (`GDMA`/`HDMA`), HBlank-triggered scheduling, and CGB-specific DMA bus restrictions are not implemented yet.
