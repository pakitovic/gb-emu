...
- DMA is now modeled as a scheduler-style state machine (DMG OAM DMA mode only) with incremental `tick(tcycles)` advancement, formal mode/edge state inside the DMA module, and centralized DMA CPU access-block policy hooks (currently DMG OAM-only blocking) to reduce future HDMA/GDMA integration refactor scope.
...
- DMA scheduling scaffolding is currently DMG-only and implements only OAM DMA transfer rules; future CGB DMA modes (`GDMA`/`HDMA`), HBlank-triggered scheduling, and CGB-specific DMA bus restrictions are not implemented yet.
