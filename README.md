...
- Mode 3 window trigger comparator now queues pending restarts until a valid BG takeover boundary when FIFO pressure prevents immediate window restart.
...
- Dot-stepped OBJ fetch contention now extends Mode 3 at runtime; BG/OBJ/window takeover boundaries are dot-stepped, but some DMG fetcher bus-phase corner-cases are still approximated.
